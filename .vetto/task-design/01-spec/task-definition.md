...
Get the daemon through the long CI validation without changing the deterministic text-processing results from the shipped revisions. Do not work around the failure by raising memory limits, weakening validation, modifying validation runners, or adding explicit garbage-collection triggers to the daemon.
