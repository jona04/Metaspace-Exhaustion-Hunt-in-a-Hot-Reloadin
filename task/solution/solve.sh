...
thread_bridge = Path("/workspace/textd/src/main/java/io/harbor/textd/core/ThreadBridge.java")
...
# Fix 2: add a ThreadLocal cleanup hook and run it on each worker during shutdown
bridge_text = thread_bridge.read_text()
old_current = """    static WorkerHandle current(TimelineLedger ledger) {\n        return ROUTES.get().cursorFor(ledger);\n    }\n"""
new_current = """    static WorkerHandle current(TimelineLedger ledger) {\n        return ROUTES.get().cursorFor(ledger);\n    }\n\n    static void clear() {\n        ThreadCache routeState = ROUTES.get();\n        routeState.close();\n        ROUTES.remove();\n    }\n"""
if old_current not in bridge_text:
    raise SystemExit("expected thread bridge snippet not found")
thread_bridge.write_text(bridge_text.replace(old_current, new_current))
